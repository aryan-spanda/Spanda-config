...
generate_simple_app() {
    ...
    local tenant=$(parse_yaml "$platform_config" '.app.tenant')
    ...
    # Validate tenant field - REQUIRED for multi-tenant deployments
    if [[ -z "$tenant" ]]; then
        warn "⚠️  TENANT FIELD MISSING: 'app.tenant' field not specified in platform-requirements.yml"
        warn "   For multi-tenant deployments, add: app.tenant: 'your-tenant-name'"
        warn "   Falling back to 'spanda-applications' project for backward compatibility"
        tenant="legacy"
        local target_namespace=$(echo "$app_name-$environment" | tr '[:upper:]' '[:lower:]')
        local project_name="spanda-applications"
    else
        local target_namespace="${tenant}-${environment}"
        local project_name="$tenant"
    fi
    ...
    log "🏢 Tenant: $tenant | Target Namespace: $target_namespace" >&2
    ...
    log "� Tenant: $tenant | Namespace: $target_namespace" >&2
...
metadata:
  ...
  annotations:
    ...
    app.spanda.ai/generator: "tenant-aware-deployment"
    app.spanda.ai/tenant: $tenant
    app.spanda.ai/target-namespace: $target_namespace
...
spec:
  project: $project_name  # Use tenant-specific project or fallback to spanda-applications
  source:
    repoURL: $repo_url
...
PARAM_EOF
  ...
  destination:
    ...
    namespace: $target_namespace  # Deploy to tenant-specific namespace
  syncPolicy:
    automated:
      selfHeal: true
      prune: true
  ...
  info:
    - name: 'Generated By'
      value: 'Tenant-Aware Microservices Generator'
    - name: 'Tenant'
      value: $tenant
